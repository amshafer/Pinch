//====- LowerToLLVM.cpp - Lowering from Pinch+Affine+Std to LLVM ------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements a partial lowering of Pinch operations to a combination of
...
//===----------------------------------------------------------------------===//
// PinchToLLVM RewritePatterns
//===----------------------------------------------------------------------===//

namespace {

/// Lowers `pinch.print` to a loop nest calling `printf` on each of the individual
/// elements of the array.
class PrintOpLowering : public ConversionPattern {
public:
  explicit PrintOpLowering(MLIRContext *context)
      : ConversionPattern(pinch::PrintOp::getOperationName(), 1, context) {}
...
//===----------------------------------------------------------------------===//
// PinchToLLVMLoweringPass
//===----------------------------------------------------------------------===//

namespace {
struct PinchToLLVMLoweringPass
    : public PassWrapper<PinchToLLVMLoweringPass, OperationPass<ModuleOp>> {
  void runOnOperation() final;
};
} // end anonymous namespace

void PinchToLLVMLoweringPass::runOnOperation() {
  ...
  // The only remaining operation to lower from the `pinch` dialect, is the
  // PrintOp.
...
std::unique_ptr<mlir::Pass> mlir::pinch::createLowerToLLVMPass() {
  return std::make_unique<PinchToLLVMLoweringPass>();
}
